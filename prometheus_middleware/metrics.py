...
def http_requested_json_code_total() -> Callable[[Info], None]:
    METRIC = Counter(
        "http_requested_json_code_total",
        "Number of times result json code.",
        labelnames=("result_code",)
    )

    def instrumentation(info: Info) -> None:
        # code = set()
        body_str = info.response.body
        code_ss = re.findall(r'"code":(\d+),', str(body_str))
        if code_ss:
            METRIC.labels(code_ss[0]).inc()
...
def default(
    ...
    * `http_request_duration_seconds` (`handler`):
    ...
    JSON_CODE_TOTAL = Counter(
        "http_requested_json_code_total",
        "Number of times result json code.",
        labelnames=("handler", "result_code",),
        namespace=metric_namespace,
        subsystem=metric_subsystem,
    )
    ...
    def instrumentation(info: Info) -> None:
        ...
        code_ss = re.findall(r'"code":(\d+),', str(body_str))
        if code_ss:
            JSON_CODE_TOTAL.labels(info.modified_handler, code_ss[0]).inc()
